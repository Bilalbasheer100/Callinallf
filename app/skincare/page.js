...
export default function SkinCareSection() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const { data } = await axios.get('/api/products?category=Skin Care');
        setProducts(data);
      ...
      <div className="max-w-6xl mx-auto">
          ...
          {filteredProducts.map((product) => (
            ...
            >
              ...
              <div className="flex justify-between items-center w-full px-4 mb-4">
                ...
                <p className="text-sm text-gray-500">Skin Care</p>
              </div>
        ...
        {filteredProducts.length === 0 && (
          <p className="text-center text-gray-600 mt-8">
            No skin care products match your search criteria.
          </p>
        )}
      </div>
    </div>
